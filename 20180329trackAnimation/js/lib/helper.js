...
var en = process.env.NODE_ENV, // 当前的打包环境
    ...
    mRoot = process.env.MiddleRoot, // 中间层或后端老站的接口地址
    ...
    helper = {
        ...
        /**
         * @description 使用GET方式请求中间层接口，并使用JSONP的方式获取接口返回值
         * @param {string} apiName 接口名称
         * @param {object} data 接口所需参数
         * @param {function?} callback 接口请求完成后的回调方法 (isOk, res) => {} 
         * @param {object?} options 其它非接口参数，如timeout、addCheckPara等
         */
        middleJSONP: function (apiName, data, callback, options) {
            if (!options) options = {};
            options["dataType"] = "jsonp";
            this.middleGet(apiName, data, callback, options);
        },
        ...
        /**
         * @description 使用GET方式请求后端接口，并使用JSONP的方式获取接口返回值
         * @param {string} apiName 接口名称
         * @param {object} data 接口所需参数
         * @param {function?} callback 接口请求完成后的回调方法 (isOk, res) => {} 
         * @param {object?} options 其它非接口参数，如timeout等
         */
        backJSONP: function (apiName, data, callback, options) {
            if (!options) options = {};
            options["dataType"] = "jsonp";
            this.backGet(apiName, data, callback, options);
        },
        ...
        */
        initStat: function (refid, baiduCode) {
            this.getUserIden((userIden) => {
                ...
                loader.loadJsByUrl("//vstlog.17usoft.com/vst.ashx");
            });
        ...
        */
        ...
        getAuthUrl: function (url, mustLogin, targetPlatId, getUserInfo) {
            ...
            if (platId == this.plat.app.id) {
                ...
                url = "https://wx.17u.cn/flight/{0}.html?url={1}".zjFill(getUserInfo ? 'getwxuserinfo' : 'getopenid', encodeURIComponent(url));
                authUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3827070276e49e30&redirect_uri={0}&response_type=code&scope={1}&state=123#wechat_redirect';
        ...
        getBuyTicketUrl: function (targetPlatId) {
            ...
            if (platId == this.plat.app.id) {
                ...
                url = 'https://wx.17u.cn/home';
            }
            if (url) {
                if (platId == this.plat.wx.id) {
                    url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3827070276e49e30&redirect_uri=https://wx.17u.cn/home/index.html&response_type=code&scope=snsapi_base&state=123#wechat_redirect";
                } else {
        ...
        getCardCouponsListUrl: function (targetPlatId) {
            var url, prot = location.protocol,
                platId = targetPlatId == null ? this.getCurrPlat().id : targetPlatId;
            if (platId == this.plat.wx.id) {
                url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3827070276e49e30&redirect_uri=https://wx.17u.cn/flight/getopenid.html?url=https://wx.17u.cn/pub/mycard&response_type=code&scope=snsapi_base&state=123#wechat_redirect';
            } else if (platId == this.plat.qq.id) {
